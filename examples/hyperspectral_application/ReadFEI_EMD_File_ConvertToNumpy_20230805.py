...
plt.rcParams[
    "toolbar"
] = "toolbar2"  # this line hides the graphics tool bar from the menu
...
SDataX_Mem = np.zeros(
    TotalChannelMax, dtype="float32"
)  # create an empty Energy list for energy (x) axis
SDataY_Mem = np.zeros(
    TotalChannelMax, dtype="float32"
)  # create an empty Energy list for energy (x) axis
...
try:  # this is a simple test routine  to see if a Group Exists
    ...
    for nList in DataSubGroupImageList:
        ...
        if Image_Data_Dimensions[2] != 0:
            ...
            Image_MetaData_Dict = json.loads(
                Image_MetaData_Text.rstrip("\x00")
            )  # \x00 is a NULL

            Param_Mem["Instrument"] = Image_MetaData_Dict["Instrument"][
                "InstrumentClass"
            ]
            Param_Mem["AcceleratingVoltage"] = Image_MetaData_Dict["Optics"][
                "AccelerationVoltage"
            ]
            Param_Mem[
                "AcceleratingVoltageUnits"
            ] = "V"  # FEI instruments list this in Volts not kV
            Param_Mem["SpotSize"] = Image_MetaData_Dict["Optics"]["SpotIndex"]
            Param_Mem["PixelSize"] = Image_MetaData_Dict["BinaryResult"]["PixelSize"][
                "width"
            ]
            Param_Mem["PixelUnits"] = Image_MetaData_Dict["BinaryResult"]["PixelUnitX"]
            #
            #
            Param_Mem["ScreenCurrent"] = Image_MetaData_Dict["Optics"][
                "LastMeasuredScreenCurrent"
            ]
            ...
            Param_Mem["STEMMag"] = Image_MetaData_Dict["CustomProperties"][
                "StemMagnification"
            ]["value"]
            Param_Mem["PixelSize"] = Image_MetaData_Dict["BinaryResult"]["PixelSize"][
                "width"
            ...
            SImageFrame = np.array(
                Image_Data[
                    0 : Image_Data_Dimensions[0], 0 : Image_Data_Dimensions[1], 0
                ],
                dtype=np.float32,
            )
            SImage = SImageFrame
            ...
            if Image_Data_Dimensions[2] > 1:  # if there are multiple frames add them up
                for index in range(1, Image_Data_Dimensions[2] - 1):
                    ...
                    SImageFrame = np.array(
                        Image_Data[
                            0 : Image_Data_Dimensions[0],
                            0 : Image_Data_Dimensions[1],
                            index,
                        ],
                        dtype=np.float32,
                    )
                    ...
                    print(
                        ...
                        index,
                        "-",
                        round(index * 100 / (Image_Data_Dimensions[2] - 1), 3),
                        "%",
                        end="\r",
                    )  # , "Intensity Limits : ", imagemin2, imagemax2, end='\r')
        ...
        print(
            "    Frame ",
            Image_Data_Dimensions[2],
            "Intensity Limits : ",
            imagemin,
            imagemax,
        )
        if (
            int(Image_Data_Dimensions[2]) > 2
        ):  # this was a multi-frame image do you want to store it?
        ...
        figname = (
            str(FileSName[FNlength]) + " : Image-" + str(tempindex) + " : " + str(nList)
        )
        ...
        plt.xticks([]), plt.yticks([])  # this turns off tick marks
        plt.imshow(
            SImage, cmap="gray", origin="upper"
        )  # what to plot note origin is funky
        plt.colorbar(shrink=0.8)  # intensity bar on the side smaller than the figure
        plt.show()
...
try:  # this is a simple test routine  to see if a Group Exists
    ...
    for nList in DataSubGroupSpectrumList:
        print(
            "    Spectrum ID-", tempindex, ":", nList
        )  # dumps some info to the screen
        ...
        Param_Mem["Instrument"] = Spectrum_MetaData_Dict["Instrument"][
            "InstrumentClass"
        ]
        Param_Mem["AcceleratingVoltage"] = Spectrum_MetaData_Dict["Optics"][
            "AccelerationVoltage"
        ]
        Param_Mem[
            "AcceleratingVoltageUnits"
        ...
        #
        #
        Param_Mem["ScreenCurrent"] = Spectrum_MetaData_Dict["Optics"][
            "LastMeasuredScreenCurrent"
        ]
        ...
        Param_Mem["STEMMag"] = Spectrum_MetaData_Dict["CustomProperties"][
            "StemMagnification"
        ]["value"]
        ...
        while DetectorMetaDataValue["DetectorType"] != "AnalyticalDetector":
            ...
            print(
                DetectorIndex,
                "Detector-" + str(DetectorIndex),
                DetectorMetaDataValue["DetectorName"],
                DetectorMetaDataValue["DetectorType"],
            )  # check this to make sure it is correct.
            DetectorIndex = (
                DetectorIndex + 1
            )  # it wasn't the XEDS detector so increement and try the next
            DetectorMetaDataValue = DetectorDir["Detector-" + str(DetectorIndex)]
    ...
    print(
        "Max channel info =",
        chnmax,
        Spectrum_Data_Dimensions[0],
        Spectrum_Data_Dimensions[1],
    )
    evch = 5  # this should be read from the metadata it will change with instrument
    zerochannelenergyoffset = (
        -479.0021
    ...
    for index in range(
        0, chnmax - 1
    ):  # this loop simply fills the Energy (X axis array)
        EnergyValue = (index * evch + zerochannelenergyoffset) / 1000.0  # keV
    ...
    if HideZeroStrobe == "True":
        ZeroStrobeShift = int(
            ZeroStrobeWidth / Param_Mem["XIncrement"]
        )  # want to move at least 100eV
    chanzerostrobe = int(
        chanzerostrobe + ZeroStrobeShift
    )  # move 5 channels off zero strobe channel
...
try:  # this is a simple test routine  to see if a Group Exists
    ...
    for nList in DataSubGroupSpectrumImageList:
        ...
        print("    ----------------------------------")
        Spectrum_ImageSettings = inputfile["Data"]["SpectrumImage"][nList][
...
try:  # this is a simple test routine  to see if a Group Exists
    ...
    print(
        "Number of SpectrumStream Records, =", len(DataSubGroupSpectrumStreamList), "\n"
    )
    ...
    for nList in DataSubGroupSpectrumStreamList:
        ...
        print("    ----------------------------------")
        Spectrum_StreamSettings = inputfile["Data"]["SpectrumStream"][nList][
            "AcquisitionSettings"
        ]
    ...
    try:
        ...
        ZDimension = Spectrum_Data_Dimensions[
            0
        ]  # this is the number of channels in a spectrum
    except ValueError as err:
        print(
            "XDimension, YDimension, ZDimension : ", XDimension, YDimension, ZDimension
        )
    ...
    tempindex = 0
    print(
        "\nStream Data Info = XDimension, YDimension, ZDimension, Events:",
        XDimension,
        YDimension,
        ZDimension,
        Spectrum_Stream_Data_Dimensions[0],
    )

    start = time.time()  # measuring the elasped time with this
    Data = np.zeros(
        Spectrum_Stream_Data_Dimensions[0], dtype=np.uint16
    )  # create an zero array for the Stream Data
    end0 = time.time()
    print(
    ...
    if ans != ("N" and "n"):
        ...
        if ans != ("N" and "n"):
            ...
            Data = Spectrum_Stream_Data[
                0 : Spectrum_Stream_Data_Dimensions[0], 0
            ]  # fill the array
    ...
    EventPixels = np.count_nonzero(
        Data == 65535
    )  # count the pixel advance flags this should be equal to the number of points* Frames
    FramePixels = (
        XDimension * YDimension
    )  # this is suppose to be the number of pixel in one Frame
    if (Image_Data_Dimensions[0] == 0) or (Image_Data_Dimensions[1] == 0):
        Frames = 0
    else:
        Frames = EventPixels / (
            Image_Data_Dimensions[0] * Image_Data_Dimensions[1]
        )  # this should be an integer
    print(
        "\nPixels in the Data Cube = ",
        XDimension,
        " x ",
        YDimension,
        " x ",
    ...
    print(
        "Signal Events in Stream File = ",
        EventPixels,
        "\nTotal Events in Stream File = ",
        Spectrum_Stream_Data_Dimensions[0],
        "\nFrames in Stream File = ",
        Frames,
    )

    end2 = time.time()  # measuring the elasped time with this
    print(
        "Elapsed Time #2 to Count the Pixels = ",
        (end2 - end1),
    ...
    if Frames != 0:
        ...
        FirstFrame = 1
        ansStartFrame = input(
            "Enter Start Frame you want to convert  (Exit=0, Otherwise Start @ Frame Number): "
        )
        ...
        if ansStartFrame == "" or ansStartFrame == str(Frames):
            ...
            if FirstFrame > 0 and LastFrame == 0:
                ansLastFrame = input(
                    "Enter Last Frame you want to convert  (Default = "
                    + str(Frames)
                    + "): "
                )
            if ansLastFrame == "":
            ...
            print(
                "\nCreating  a TotalArea-Sum-Spectra from ",
                FirstFrame,
                "-",
                LastFrame,
    ...
    HyperCubeData = np.zeros(
        (XDimension, YDimension, ZDimension)
    )  # this is our data cube
    ...
    HyperSpectrumEnergy = np.zeros(
        4096
    )  # initialize the Energy Axis array for the sum spectrum array
    ...
    for (
        ...
        Data
    ):  # this converts the Stream Data File into a HyperCube data file 640x640x41 test = 1.5 minutes
        channel = int(event)
        ...
        if (
            channel != 65535
        ):  # we have a real signal event  65535 means move to the next channel
            if XYZFrameNumber >= FirstFrame - 1:
                HyperSpectrumSumData[
                    channel
                ] += 1  # add a count to the event channel this is a SUM spectrum
                HyperCubeData[
                    XPixel, YPixel, channel
                ] += 1  # This adds one count to the spectrum Channel at Xpixel,Ypixel
        else:
            #            if channel == 65535: # we have a pixel increment flag not an signal event
            if (
                YPixel < YDimension
            ):  # Note:  the data in teh input array is Y vs X  not X vs Y (C vs Fortan style??)
                YPixel += 1
            ...
            if XPixel == XDimension:  # we are at the Y limit reset Y increment Frame
                ...
                if XYZFrameNumber < FirstFrame:
                    print(
                        "\rFrame :", XYZFrameNumber, " skipped", end="\r"
                    )  # this is a simple counter to show progress
                else:
                    print(
                        "\rFrame :",
                        XYZFrameNumber,
                        ...
                        Frames,
                        ") -",
                        round(XYZFrameNumber * 100 / LastFrame, 2),
                        "%",
                        end="\r",
                    )  # this is a simple counter to show progress
    ...
    print(
        "\nElapsed time to create HSI Sum Spectra =",
        (end3 - end2) / 60,
        " minutes\n",
        len(Data) / (100000 * (end3 - end2)),
        " MegaEvents/sec",
    )
    #    close(FileName)
    if LastFrame != 0:  # only draw if there are Frames read and  stored
        ...
        if ans == "Y" or ans == "y" or ans == "":
            ...
            ParamFile = open(
                ...
                + "-"
                + str(LastFrame)
                + "Frames-HSDataCube_64bit.npy.dict",
                "w",
            )
            ParamFile.write(json.dumps(Param_Mem))
            ParamFile.close()
            ...
            # print (Param_Mem)
            print(
                "Elapsed Time to create backup files = ", (end5 - end4) / 60, " minutes"
            )
        ...
        if HideZeroStrobe == "True":
            ZeroStrobeShift = int(
                ZeroStrobeWidth / Param_Mem["XIncrement"]
            )  # want to move at least strobe width
            chanzerostrobe0 = int(
                chanzerostrobe0 + ZeroStrobeShift
            )  # move 5 channels off zero strobe channel
        ...
        evch = Param_Mem[
            "XIncrement"
        ]  # this should be read from the metadata it will change with instrument
        zerochannelenergyoffset = Param_Mem[
            "XOffset"
        ]  # these should be read from the metadata it will change with instrument
        #       Energymax=20.# 20 keV for this test program.
        for index in range(0, channelmax - 1):
            EnergyValue = (index * evch + zerochannelenergyoffset) / 1000.0  # keV
            HyperSpectrumEnergy[index] = EnergyValue
            # index should be multipled by ev/channel + zerochannelenergyoffset
        ...
        HyperSpectrumSumFigure = plt.figure(
            num="EMD-HSCube-SumSpectrum-Figure", figsize=(6, 4.5)
        )
        plt.cla()
        plt.plot(
            HyperSpectrumEnergy[ZeroEnergyChannel : MaxEnergyChannel - 1],
            HyperSpectrumSumData[ZeroEnergyChannel : MaxEnergyChannel - 1],
        )
        plt.xticks(np.arange(EXMin, EXMax, step=xstep))  # this turns off tick marks
        plt.title(
            str(FileSName[FNlength])
            + " :\n "
            + str(FirstFrame)
            + "-"
            + str(LastFrame)
            + " Frame(s) Sum Spectrum"
        )
        plt.ylabel("Counts")
        plt.xlabel("Energy-keV")
        if YMin <= 0:
