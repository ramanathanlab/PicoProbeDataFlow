...
#   better for AEM
#    
    def __init__(self, counts, positions, DataCubeParameters, fig=None, pos_order=None,
                 norm=None):
...
#            print (key, ": ", value)
        ...
        self.counts = counts[:] #/ norm  I want a sum spectrum not normalized I removed the /norm
        
        # this fills in the calibrated Energy Axis
        
        for Enchannel in range (0,DataCubeParameters['NChannels']): 
            self.energy [Enchannel]= (DataCubeParameters['XOffset'] +  DataCubeParameters['XIncrement']*Enchannel)/1000.
...
#        print (Enchannel, self.energy [Enchannel])
        ...
        if fig is None:
            import matplotlib.pyplot as plt
            fig = plt.figure(tight_layout=True, figsize=(15,5)) # I like bigger figures added fig size
        # clear the figure
        fig.clf()
...
#        self.ax_spec.set_ylim(yspecmin,yspecmax)
        ...
        self._pixel_txt = self.ax_spec.annotate('HSImage average',
                                                xy=(1, 1),
                                                xytext=(0, 5),
                                                xycoords='axes fraction',
                                                textcoords='offset points',
        ...
        self.spec, = self.ax_spec.plot(self.energy[:],self.counts[:,:,:].sum(axis=(0,1)), lw=2)
...
#        self.spec, = self.ax_spec.plot(self.counts[:,:,:].sum(axis=(0,1)), lw=2)  # this was original uncalibrated x axis
    ...
    def _reset_spectrum(self):
        ...
        new_y_data = self.counts[:,:,:].sum(axis=(0, 1)) # I want the sum spectrum not a mean
...
#        offset=-479.0021 #DataCubeParameters['XOffset']
        cmin=(vmin*1000. - self.offset)/self.evchn # calculate the channel value
        cmax=(vmax*1000. - self.offset)/self.evchn # calcualte the channel value
...
#        self.markerprops = dict(markersize=7)
        self.polygon = mwidgets.PolygonSelector(event.inaxes, self._polygon_call_back,vertex_select_radius=10)# #self.lineprops, self.markerprops)
...
SDataX_Mem=np.zeros(TotalChannelMax)  # create an empty Energy list for Spectral energy (x) axis
SDataY_Mem=np.zeros(TotalChannelMax)  # create an empty Energy list for Spectral data  (y) axis
...
ParamFile=open (FileName+".dict",'r') # I defined this parameter file it is created by the translator
...
try :
    ZDimension=HyperCubeData.shape[2]  # if this value is non-zero then it is a 3D cube otherwise it is a 2D image

except:
    ZDimension=0
    print ("HyperCubeData Dimensions : [",HyperCubeData.shape[0]," x ", HyperCubeData.shape[1]," ] ") # this is the shape info of the file
...
if ZDimension != 0 : # this is a 3D cube
    # this is a data cube 
    print ("\nHyperCubeData Dimensions : [",HyperCubeData.shape[0]," x ", HyperCubeData.shape[1]," x ", HyperCubeData.shape[2]," ] ") # this is the shape info of the file
    
    print ('Opening File for Analysis')
    FSize=XDimension*YDimension*ZDimension
    if FSize > 1e9 : print ('This is a large file ',2*FSize/1e9, 'Gb it will take time') #
    ...
    XYpositionarray=np.empty(shape=(2,XDimension,YDimension)) # the shape of the position array
...
#
    ...
    SDataX_Mem=AEMGraphicObject.SDataX   # this is a one time read need a loop or something to write it to a file
